...
def reward_func(reward_type, lin_result, rl_result):
    ...
    if reward_type == 'op':
        if clk_result >= 1 and cost_result < 1:
            a = 1.5
            b = 0.5
        elif clk_result >= 1 and cost_result >= 1:
            a = 1
            b = 1
        elif clk_result < 1 and cost_result >= 1:
            a = 0.5
            ...
            a = 1.2
            b = 0.8
        r = a * clk_result - b * cost_result
        # r = r / 100
        # print("点击：{}, hb点击：{}, 花费：{}, hb花费：{}".format(clks, hb_clks, cost, hb_cost))
        # print(clk_result, cost_result, r)
        # print()
        return r
                ...
                if test_win_clk > best_win_clk:
                    ...
                    train_bid_action_df = pd.DataFrame(data=train_bid_action,
                                                       columns=['clk', 'pctr', 'market_price', 'day', 'time_fraction',
                                                                'bid_price', 'win'])
                    train_bid_action_df.to_csv(
                        bid_action_path + '/train_' + str(budget_para) + '.csv',
                        index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--result_path', type=str, default='add-log-result')
    ...
    parser.add_argument('--budget_para', nargs='+', default=[4])#[2, 4, 8, 16]
    ...
    parser.add_argument('--tensorboard_prefix', type=str, default='new')  # tensorboard

    args = parser.parse_args()
    config = vars(args)

    str_time = time.strftime('%Y-%m-%d %H-%M-%S', time.localtime())
    config['result_path'] = config['result_path'] + '-camp={}-seed={}-{}'.format(config['campaign_id'], config['seed'], str_time)
    ...
    for i in budget_para_list:
        RL = PADDPG(
            ...
            seed=config['seed'],
            time=str_time,
            tensorboard_prefix=config['tensorboard_prefix']
        )
