...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim: int, action_dim: int, size: int, batch_size: int = 32):# obs_dim = featrue_num = 24=6*4
        """Initialization.""" #size memory_size 100000
...
class PADDPG:
    def __init__(
            ...
            tau_c: float = 0.0001,
            lr_a: float = 1e-3,
            lr_c: float = 2e-3, # todo 这里和论文中的不一样 论文里是2e-3 原来是1e-3
            initial_random_steps: int = 128,
            seed: int = 1,
            time: str = '',
            tensorboard_prefix: str = 'tensorboard'
    ):
        ...
        self.num_actions = self.action_space[0].n # 2
        self.action_parameter_sizes = np.array(
            [self.action_space.spaces[i].shape[0] for i in range(1, self.num_actions + 1)]) # 两个连续动作空间大小（1，1）
        self.action_parameter_size = int(self.action_parameter_sizes.sum()) # 1+1 = 2
        self.action_max = torch.from_numpy(np.ones((self.num_actions,))).float().to(self.device) # （1，1）
        self.action_min = torch.from_numpy(np.zeros((self.num_actions,))).float().to(self.device)# （0，0）
        self.action_range = (self.action_max - self.action_min).detach() # 1-0 = 1
        ...
        self.writer = SummaryWriter('{}-camp={}-budget={}-seed={}-{}/'.format(tensorboard_prefix,camp, budget, seed, time))
    ...
    def select_action(self, state: np.ndarray):
        ...
        with torch.no_grad():
            ...
            all_action_parameters = np.clip(all_action_parameters, 0., 0.99)# todo 这里直接截断？
    ...
    def update_model(self) -> Tuple:
        ...
        if self.clip_grad > 0:
            nn.utils.clip_grad_norm_(self.critic.parameters(), self.clip_grad) # 剪裁梯度，防止梯度爆炸
