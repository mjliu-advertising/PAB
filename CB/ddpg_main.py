...
def bid(data, budget, **cfg):
    ...
    data['bid_price'] = np.clip((data['pctr']* cfg['omiga']).astype(int), 0, 300)
...
def rtb(data, budget_para, RL, config, train=True):
    ...
    for day_index, day in enumerate(data['day'].unique()):
        ...
        for slot in range(0, time_fraction):
            ...
            # slot_lin_result = bid(slot_data, day_budget[-1],omiga = omiga)
            slot_lin_result = []
            slot_rl_result = bid(slot_data, day_budget[-1],omiga = slot_rl_bid_para)
    ...
    episode_spend_episode_win_imps = sum(episode_spend) / sum(episode_win_imps) if sum(episode_win_imps) != 0 else 0
    episode_spend_episode_win_clks = sum(episode_spend) / sum(episode_win_clks) if sum(episode_win_clks) != 0 else 0
    print(
        result + "：点击数 {}, 真实点击数 {}, pCTR {:.4f}, 真实pCTR {:.4f}, 赢标数 {}, 真实曝光数 {}, 花费 {}, CPM {:.4f}, CPC {:.4f}, 奖励 {:.2f}".format(
            ...
            sum(episode_spend),
            episode_spend_episode_win_imps,
            episode_spend_episode_win_clks,
            sum(episode_reward)
        )
    )
    ...
    return episode_record, episode_action, episode_bid_action, int(sum(episode_win_clks))
...
def main(budget_para, RL, config):
    ...
    for epoch in range(config['train_epochs']):
        ...
        train_record, train_action, train_bid_action,train_win_clk = rtb(train_data, budget_para, RL, config)
        test_record, test_action, test_bid_action, test_win_clk = rtb(test_data, budget_para, RL, config, train=False)
        ...
        if config['save_bid_action']:
            ...
            if config['save_best_win_clk_bid_action']:
                if test_win_clk > best_win_clk:
                    best_win_clk = test_win_clk
                    train_bid_action_df = pd.DataFrame(data=train_bid_action,
                                                       columns=['clk', 'pctr', 'market_price', 'day', 'time_fraction',
                                                                'bid_price', 'win'])
                    train_bid_action_df.to_csv(bid_action_path + '/train_' + str(budget_para)  + '.csv',
                                               index=False)

                    test_bid_action_df = pd.DataFrame(data=test_bid_action,
                                                      columns=['clk', 'pctr', 'market_price', 'day', 'time_fraction',
                                                               'bid_price', 'win'])
                    test_bid_action_df.to_csv(bid_action_path + '/test_' + str(budget_para)  + '.csv',
                                              index=False)
                    create_csv_file(bid_action_path + '/info_' + str(budget_para) + '_.csv',
                                    ['budget_para', 'epoch', 'best_win_clk'],
                                    [budget_para, epoch, best_win_clk])

                    columns = ['clks', 'real_clks', 'pctr', 'real_pctr', 'imps', 'real_imps', 'spend', 'CPM', 'CPC',
                               'reward']
                    train_record_df = pd.DataFrame(data=[train_record], columns=columns)
                    train_record_df.to_csv(bid_action_path + '/train_episode_results_' + str(budget_para) + '.csv')

                    train_action_df = pd.DataFrame(data=[train_action])
                    train_action_df.to_csv(bid_action_path + '/train_episode_actions_' + str(budget_para) + '.csv')

                    test_record_df = pd.DataFrame(data=[test_record], columns=columns)
                    test_record_df.to_csv(bid_action_path + '/test_episode_results_' + str(budget_para) + '.csv')

                    test_action_df = pd.DataFrame(data=[test_action])
                    test_action_df.to_csv(bid_action_path + '/test_episode_actions_' + str(budget_para) + '.csv')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_bid_action', type=bool, default=True)
    parser.add_argument('--reward_type', type=str, default='clk', help='op, nop_2.0, clk')
    parser.add_argument('--device', type=str, default='cuda')

    parser.add_argument('--save_best_win_clk_bid_action', type=bool, default=True)
    ...
    # 飞书机器人的Webhook URL，替换为你自己的URL
    webhook_url = 'https://open.feishu.cn/open-apis/bot/v2/hook/e303365b-5773-4cc8-a0f2-a3d1fbe84112'

    # 要发送的消息内容
    message = {
        "msg_type": "text",
        "content": {
            "text": "来自3080机器，CB--ddpg_main任务完成了,请及时查看！"
        }
    }
    ...
    if response.status_code == 200:
        ...
        print("消息发送失败，HTTP响应代码：", response.status_code)