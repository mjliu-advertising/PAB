...
def rtb(data, budget_para, RL, config, train=True):
    ...
    return episode_record, episode_action, episode_lambda, episode_bid_action, int(sum(episode_win_clks))
...
def main(budget_para, RL, config):
    ...
    for epoch in range(config['train_epochs']):
        ...
        train_record, train_action, train_lambda, train_bid_action, train_win_clk = rtb(train_data, budget_para, RL, config)
        test_record, test_action, test_lambda, test_bid_action, test_win_clk = rtb(test_data, budget_para, RL, config, train=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--result_path', type=str, default='addLog-result')
    ...
    message = {
        "msg_type": "text",
        "content": {
            "text": "来自3080机器，drlb_main_addLog.py任务完成了,请及时查看！"
        }
    }
